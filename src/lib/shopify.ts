...
const SHOPIFY_STORE_PERMANENT_DOMAIN = 'u70w6n-v8.myshopify.com';
const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
const SHOPIFY_STOREFRONT_TOKEN = '6116c532a9dc8d7af607a8b9b53dddd7';
...
export async function storefrontApiRequest(query: string, variables: Record<string, unknown> = {}) {
  ...
  if (response.status === 402) {
    toast.error("Shopify: Betalning krävs", {
      description: "Ditt Shopify-konto behöver uppgraderas till en betald plan.",
    });
    return;
  }
...
export async function fetchProducts(first: number = 10, query?: string) {
...
function formatCheckoutUrl(checkoutUrl: string): string {
  try {
    ...
    // Replace custom domain with myshopify.com domain to avoid 404 on custom domains
